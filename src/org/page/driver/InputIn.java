...
public class InputIn extends PageApi {
	private final String enterValue;

	public InputIn(String enterValue, String object_repository) {
	...
	public String execute() throws StopTest {
		 ...
		 * String scriptInput =
		 * "function getElementByXpath(path) { return document.evaluate(path, document, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null).singleNodeValue;}; getElementByXpath(\"actualxpath\").value = \"actualvalue\" ;"
		...
		try {
			...
			try {
				...
				ele.clear();
				putString(enterValue);
				String scriptResult2 = (String) executor.executeScript(scriptResult);
				if (!scriptResult2.toUpperCase().equals(enterValue.toUpperCase())) {
					Thread.sleep(3000);
					ele.clear();
					putString(enterValue);
				}
